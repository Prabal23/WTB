...
class ReplyProfilePostPage extends StatefulWidget {
  final comList;
  final statusID;
  final index;
  ReplyProfilePostPage(this.comList, this.statusID, this.index);
  @override
  ReplyPageState createState() => ReplyPageState();
}

class ReplyPageState extends State<ReplyProfilePostPage> {
  var repList;
  var userData;
  ...
  Future commentSend(index) async {
    ...
    if (res.statusCode == 200) {
      repEditor.text = "";
      profileReplyList[index]['count'] += 1;
  ...
  Future deleteStatus(int id, int index) async {
    ...
    if (body == 1) {
      setState(() {
        //isDel.add("$id");
        idx = -1;
        profileReplyList[widget.index]['count'] -= 1;
