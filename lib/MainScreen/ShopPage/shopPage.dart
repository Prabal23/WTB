...
class ShopPage extends StatefulWidget {
  ...
  ShopPage(this.shopId, this.userID);
  @override
...
class _ShopPageState extends State<ShopPage> {
  ...
  Future loadData() async {
    ...
    var postresponse =
        await CallApi().getData2('getShopDetails/${widget.shopId}');
    ...
    uID = productList.shopDetails.sellerId;

    setState(() {
      loading = false;

      if (productList.shopDetails.verified != null) {
        if (productList.shopDetails.verified.status == "Pending") {
          _isPending = true;
        } else {
          _isVerified = true;
        }
  ...
  Future verifyShop() async {
    setState(() {
      loading = true;
    });

    var data = {"type": "shop", "shop_id": widget.shopId};
    //await Future.delayed(Duration(seconds: 3));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: (loading == true && proloading == true) || productList == null
                  ...
                  SliverToBoxAdapter(
                    child: Column(
                      children: <Widget>[
                            ...
                            child: Row(
                              children: <Widget>[
                                ...
                                GestureDetector(
                                  onTap: () {
                                    setState(() {
                                      print("productList.shopDetails.verified");
                                      print(productList.shopDetails.verified);
                                      if (uID == widget.userID) {
                                        if (productList.shopDetails.verified ==
                                            null) {
                                          verifyShop();
                                        }
                                      }
                        ...
                        productList.shopDetails.seller.userName ==
                                userData['userName']
                            ? Container()
                            : GestureDetector(
                                onTap: () {
                                  Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                          builder: (context) =>
                                              FriendsProfilePage(
                                                  productList.shopDetails.seller
                                                      .userName,
                                                  2)));
                                },
                                child: Container(
