...
class StatusCommentPage extends StatefulWidget {
  final feedId;
  final index;
  StatusCommentPage(this.feedId, this.index);
  @override
  _CommentPageState createState() => _CommentPageState();
}

class _CommentPageState extends State<StatusCommentPage> {
  var comList;
  var userData;
  ...
  Future loadComments() async {
    ...
    setState(() {
      ...
      for (int i = 0; i < comList.comments.length; i++) {
        print("comList.comments[i].replay.totalReplyCount");
        print(comList.comments[i].replay.totalReplyCount);
        statusPostRepCount
                      ...
                      children: <Widget>[
                        ...
                        GestureDetector(
                          onTap: () {
                            ...
                            statusPostRepCount.add({'count': 0});
                          },
                          child: Container(
                            decoration: BoxDecoration(
                                color: back,
  ...
  Future commentSend(index) async {
    ...
    if (res.statusCode == 200) {
      if (commentList.length != 0 && statusPostRepCount.length != 0) {}
      commentList.removeAt(commentList.length - 1);
      statusPostRepCount.removeAt(statusPostRepCount.length - 1);
      comEditor.text = "";
      setState(() {
        statusPostComCount[widget.index]['count'] += 1;
        commentList.add({
          'id': "${body['comment']['id']}",
      ...
      comEditor.text = "";
      statusPostRepCount.removeAt(commentList.length - 1);
      commentList.removeAt(commentList.length - 1);
      statusPostComCount[index]['count'] -= 1;
    }
  ...
  Future deleteStatus(int id, index) async {
    ...
    if (body == 1) {
      setState(() {
        ...
        statusPostComCount[widget.index]['count'] -= 1;
        //isDeleted.add("$id");
        statusPostRepCount.removeAt(widget.index);
        ...
        sliver: SliverList(
          delegate:
            ...
            return Container(
              ...
              margin: EdgeInsets.only(
                  top: 0,
                  bottom: idx == widget.comList.length - 1 ? 20 : 0,
                  left: 20,
                  right: 10),
              child: Column(
                children: <Widget>[
                  Container(
                    margin:
                        EdgeInsets.only(left: 0, right: 0, bottom: 0, top: 10),
                          ...
                          child: Container(
                            child: Column(
                              ...
                              children: <Widget>[
                                ...
                                statusPostRepCount[index]['count'] == 0
                                    ? Container()
                                    : GestureDetector(
                                        onTap: () {
                                          Navigator.push(
