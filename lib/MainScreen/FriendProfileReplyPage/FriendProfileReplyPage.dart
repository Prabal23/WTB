...
class FriendProfileReplyPage extends StatefulWidget {
  final comList;
  final statusID;
  final index;
  FriendProfileReplyPage(this.comList, this.statusID, this.index);
  @override
  ReplyPageState createState() => ReplyPageState();
}

class ReplyPageState extends State<FriendProfileReplyPage> {
  var repList;
  var userData;
  ...
  Future commentSend(index) async {
    ...
    if (res.statusCode == 200) {
      repEditor.text = "";
      fprofileReplyList[index]['count'] += 1;
...
class CreatePostCard extends StatefulWidget {
  ...
  final statusID;
  CreatePostCard(
      this.repList, this.comList, this.userData, this.index, this.statusID);
                          ...
                          Expanded(
                            child: GestureDetector(
                              onTap: () {
                                setState(() {
                                  Navigator.of(context).pop();
                                  commentButton(widget.repList[index]['id'],
  ...
  Future deleteStatus(int id, int index) async {
    ...
    if (body == 1) {
      setState(() {
        //isDel.add("$id");
        idx = -1;
        fprofileReplyList[widget.index]['count'] -= 1;
                                                                        ...
                                                                        child:
                                                                          ...
                                                                          children: <
                                                                            ...
                                                                            GestureDetector(
                                                                              onTap: () {
                                                                                setState(() {
                                                                                  commentButton(widget.repList[index]['id'], widget.repList[index]['uId']);
                                                                                  widget.repList[index]['replyTxt'] = reply;
                                                                                  isEdit = false;
                                                                                  idx = -1;
                                                                                });
