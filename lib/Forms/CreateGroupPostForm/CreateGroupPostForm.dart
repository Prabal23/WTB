...
class CreateGroupPostForm extends StatefulWidget {
  final userData;
  final groupList;
  CreateGroupPostForm(this.userData, this.groupList);
...
class _CreateGroupPostFormState extends State<CreateGroupPostForm> {
  ...
  Future uploadImages(List<File> resultList) async {
    ...
    if (res1.statusCode == 200) {
      ...
      for (int i = 0; i < body1['uploadImages'].length; i++) {
        ids.add(body1['uploadImages'][i]['id']);
        print(body1['uploadImages'][i]['file']);
        if (body1['uploadImages'][i]['file'].contains("127.0.0.1")) {
          body1['uploadImages'][i]['file'] = body1['uploadImages'][i]['file']
              .replaceAll("127.0.0.1", "10.0.2.2");
        }
        imgs.add({"img": "${body1['uploadImages'][i]['file']}"});
        print(allImages);
        print(imgs);
      }
                    ...
                    return GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              image: DecorationImage(
                                  fit: BoxFit.contain,
                                  image: NetworkImage(img[index]['img']))),
  ...
  Future statusUpload() async {
    ...
    var res = await CallApi().postData1(data, 'post/com_post');
