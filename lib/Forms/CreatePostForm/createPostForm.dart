...
class CreatePostForm extends StatefulWidget {
  final userData;
  final page;
  CreatePostForm(this.userData, this.page);
  @override
...
class _CreatePostFormState extends State<CreatePostForm> {
  ...
  Future uploadImages(List<File> resultList) async {
    ...
    if (body1['success'] == true) {
      ...
      for (var i = 0; i < body1['uploadImages'].length; i++) {
        ...
        img.add(body1['uploadImages'][i]['file']);
      }
  ...
  Future linkCheck() async {
    ...
    if (interests == "") {
      _showMsg("Please select an interest");
      setState(() {
        isSubmit = false;
      });
    } else if (post == "" && allImages.length == 0) {
      _showMsg("Please write something/select image");
      setState(() {
        isSubmit = false;
      });
    } else if (post == "" && allImages.length != 0) {
      _showMsg("Please write something");
      setState(() {
        isSubmit = false;
      });
    } else {
      ...
      var res = await CallApi().postData1(data, 'get_link_meta');

      if (res.statusCode == 200) {
        var body = json.decode(res.body);
        print(body);
        setState(() {
          //print(body['linkMeta']['title']);
          link = post;
          //metaData = body['linkMeta'];
          metaData = json.encode(body['linkMeta']);
          statusUpload(link, metaData);
        });
  ...
  Future statusUpload(link, metaData) async {
    if (interests == "") {
      ...
      if (res.statusCode == 200) {
        ...
        return _showMsg("Something went wrong!");
      }
    }
  }
}
