...
class EditPostFormState extends State<EditPostForm> {
  ...
  void initState() {
    ...
    interests = widget.postList.interests;
    postController.text = widget.postList.data.status;
    status = widget.postList.data.privacy;
    for (int j = 0; j < widget.postList.data.images.length; j++) {
      allImages.add(widget.postList.data.images[j]);
    }
  ...
  Future statusUpload() async {
    ...
    //   'images': widget.postList.data.images
    // };

    var data = {
      'user_id': widget.userData['id'],
      'feedType': "Status",
      'sourceId': widget.postList.sourceId,
      'activitytext': post,
      'privacy': status,
      'interests': interests,
      'isEdit': true,
